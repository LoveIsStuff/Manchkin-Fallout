...
public class Terrain {
	...
	public float getHeight(int x, int z, BufferedImage image) {
		...
		height += (float) MAX_PIXEL_COLOR / 2.0f;
		height /= (float) MAX_PIXEL_COLOR / 2.0f;
	...
	public float getHeightOfTerrain(float worldX, float worldZ) {
		...
		float gridSquareSize = (float) SIZE / ((float) heights.length - 1);
		...
		float xCoord = (float) (terrainX % gridSquareSize) / gridSquareSize;
		float zCoord = (float) (terrainZ % gridSquareSize) / gridSquareSize;
		float answer;
		if (xCoord <= (1 - zCoord)) {
